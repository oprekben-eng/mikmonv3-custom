...
// get variable
  ...
  $removeippool = $_GET['remove-ippool'];
  $srv = $_GET['srv'];
  $prof = $_GET['profile'];
  $comm = $_GET['comment'];
  ...
  $pageppp = array('secrets','profiles','active','offline','billing');
...
// ppp offline
  elseif ($ppp == "offline") {
    include_once('./ppp/pppoffline.php');
  }

// ppp billing
  elseif ($ppp == "billing") {
    include_once('./ppp/billing.php');
  }
...
// ppp ip pool
  elseif ($ppp == "ippool") {
    include_once('./ppp/ippool.php');
  }

// add ppp ip pool
  elseif ($ppp == "addpool") {
    include_once('./ppp/addpool.php');
  }

// remove ip pool
  elseif ($removeippool != "") {
    echo "<b class='cl-w'><i class='fa fa-circle-o-notch fa-spin' style='font-size:24px'></i> Processing...</b>";
    $API->comm("/ip/pool/remove", array(".id" => $removeippool));
...
// agent management (admin panel)
  elseif ($agent == "manage") {
    include_once('./agent/manage_agents.php');
  }
  elseif ($agent == "add") {
    include_once('./agent/add_agent.php');
  }
  elseif ($agent == "edit") {
    include_once('./agent/edit_agent.php');
  }
  elseif ($agent == "topup") {
    include_once('./agent/topup_agent.php');
  }
  elseif ($agent == "prices") {
    include_once('./agent/prices.php');
  }
  elseif ($agent == "transactions") {
    include_once('./agent/admin_transactions.php');
  }
  elseif ($agent == "fonnte") {
    include_once('./agent/fonnte_settings.php');
  }
  elseif ($agent == "format-voucher") {
    include_once('./agent/format_voucher.php');
  }
...
if ($hotspot == "dashboard" || substr(end(explode("/", $url)), 0, 8) == "?session") {
  echo '<script>
    ...
    $("#loadppp").load("./dashboard/aload.php?session=' . $session . '&load=ppp #r_ppp");  
