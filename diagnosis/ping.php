  ...
  <div class="col-8">
    <div class="card">
      <div class="card-header">
        <h3><i class="fa fa-exchange"></i> Ping Target</h3>
      </div>
      <div class="card-body">
        <label>Alamat IP / Hostname Tujuan:</label>
        <div class="input-group">
            <input type="text" class="form-control" id="ping_target" value="8.8.8.8" placeholder="Contoh: google.com">
            <button class="btn btn-primary bg-primary" id="btn_ping" onclick="runDiagnosis('ping')">
                <i class="fa fa-play text-white"></i> <span class="text-white">Jalankan Ping</span>
            </button>
        </div>
      </div>
    </div>
...
function setTarget(ip) {
    document.getElementById("ping_target").value = ip;
}

function runDiagnosis(type) {
    var target = document.getElementById("ping_target").value;
    if(target == "") {
        alert("IP / Domain Tujuan tidak boleh kosong!");
        return;
    }
    
    document.getElementById("diag_output").innerHTML = "[admin@MikroTik] > ping " + target + " count=5\nMemulai ping dari router ke " + target + "...\nHarap tunggu...";
