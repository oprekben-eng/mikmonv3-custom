...
// get MikroTik system clock
  include_once('./lib/ros_compat.php');
  $getclock = $API->comm("/system/clock/print");
  $clock = $getclock[0];
  $timezone = safeGet($clock, 'time-zone-name', 'UTC');
...
// PRELOAD PPP DATA SO IT DOESNT DELAY
  $countallppp = $API->comm("/ppp/secret/print", array("count-only" => ""));
  if ($countallppp < 2) { $puunit = "item"; } else { $puunit = "items"; }
  $countpppactive = $API->comm("/ppp/active/print", array("count-only" => ""));
  if ($countpppactive < 2) { $paunit = "item"; } else { $paunit = "items"; }
  $countpppoffline = $countallppp - $countpppactive;
  if ($countpppoffline < 2) { $pounit = "item"; } else { $pounit = "items"; }
            ...
            <div class="box-group-icon"><i class="fa fa-calendar"></i></div>
              <div class="box-group-area">
                <span ><?= $_system_date_time ?><br>
                    <?php 
                    $dateDisplay = formatROSDate(safeGet($clock, 'date', ''));
                    echo $dateDisplay . " " . safeGet($clock, 'time', '') . "<br>
                    ".$_uptime." : " . formatDTM(safeGet($resource, 'uptime', '0s'));
                    $_SESSION[$session.'sdate'] = safeGet($clock, 'date', '');
                    ?>
                </span>
              </div>
            </div>
        ...
        <div class="box bmh-75 box-bordered">
          ...
          <div class="box-group-icon"><i class="fa fa-info-circle"></i></div>
              <div class="box-group-area">
                <span >
                    <?php
                    echo $_board_name." : " . safeGet($resource, 'board-name', 'N/A') . "<br/>
                    ".$_model." : " . safeGet($routerboard, 'model', 'N/A') . "<br/>
                    Router OS : " . safeGet($resource, 'version', 'N/A');
                    ?>
                </span>
              </div>
            </div>
      ...
      <div class="box bmh-75 box-bordered">
        <div class="box-group">
          <div class="box-group-icon"><i class="fa fa-server"></i></div>
              <div class="box-group-area">
                <span >
                    <?php
                    echo $_cpu_load." : " . safeGet($resource, 'cpu-load', '0') . "%<br/>
                    ".$_free_memory." : " . formatBytes(safeGet($resource, 'free-memory', 0), 2) . "<br/>
                    ".$_free_hdd." : " . formatBytes(safeGet($resource, 'free-hdd-space', 0), 2)
                    ?>
                </span>
                </div>
              </div>
        ...
        <div class="row">
          <div  class="col-8">
            ...
            <!-- ppp bulanan preloaded -->
            <div id="loadppp">
              <div class="row">
              <div class="card" style="margin-top: 15px;">
                <div class="card-header"><h3><i class="fa fa-wifi"></i> PPP BULANAN</h3></div>
                  <div class="card-body">
                    <div class="row">
                      ...
                      <div class="col-3 col-box-6">
                        <div class="box bg-blue bmh-75">
                          <a href="./?ppp=active&session=<?= $session; ?>">
                            <h1><?= $countpppactive; ?> <span style="font-size: 15px;"><?= $paunit; ?></span></h1>
                            <div><i class="fa fa-laptop"></i> <?= $_ppp_active ?? 'PPP Active' ?></div>
                          </a>
                        </div>
                      </div>
                      
                      <!-- Box: PPP Offline (Hijau) -->
                      <div class="col-3 col-box-6">
                        <div class="box bg-green bmh-75">
                          <a href="./?ppp=offline&session=<?= $session; ?>">
                            <h1><?= $countpppoffline; ?> <span style="font-size: 15px;"><?= $pounit; ?></span></h1>
                            <div><i class="fa fa-users"></i> <?= $_ppp_offline ?? 'PPP Offline' ?></div>
                          </a>
                        </div>
                      </div>
                      
                      <!-- Box: Generate / Tambah (Kuning) -->
                      <div class="col-3 col-box-6">
                        <div class="box bg-yellow bmh-75">
                          <a href="./?ppp=addsecret&session=<?= $session; ?>">
                            <div>
                              <h1><i class="fa fa-user-plus"></i> <span style="font-size: 15px;">Add</span></h1>
                            </div>
                            <div><i class="fa fa-user-plus"></i> PPP Secret</div>
                          </a>
                        </div>
                      </div>

                      <!-- Box: Daftar Profil (Merah) -->
                      <div class="col-3 col-box-6">
                        <div class="box bg-red bmh-75">
                          <a href="./?ppp=add-profile&session=<?= $session; ?>">
                            <div>
                              <h1><i class="fa fa-user-plus"></i> <span style="font-size: 15px;">Add Profile</span></h1>
                            </div>
                            <div><i class="fa fa-user-plus"></i> PPP Profile</div>
                          </a>
                        </div>
                      </div>
