...
// load config
  ...
  $getuprofile = safeGet($getuser[0], 'profile', 'default');


  $getprofile = $API->comm("/ip/hotspot/user/profile/print", array("?name" => "$getuprofile"));
  $getsharedu = safeGet($getprofile[0], 'shared-users', safeGet($getprofile[0], 'shared', '1'));
  $ponlogin = safeGet($getprofile[0], 'on-login', '');
...
<?php for ($i = 0; $i < $TotalReg; $i++) {;
  ...
  $username = safeGet($regtable, 'name', '');
  $password = safeGet($regtable, 'password', '');
  $profile = safeGet($regtable, 'profile', 'default');
  $timelimit = safeGet($regtable, 'limit-uptime', '');
  $getdatalimit = safeGet($regtable, 'limit-bytes-total', 0);
  $comment = safeGet($regtable, 'comment', '');
  if ($getdatalimit == 0) {
    $datalimit = "";
  } else {
